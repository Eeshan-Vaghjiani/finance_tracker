...
class MPesaParser {
  ...
  static final RegExp sendMoneyRegex = RegExp(
    r'Ksh([0-9,.]+)\s+paid\s+to\s+(.+?)\.?\s+on\s+(\d{1,2}/\d{1,2}/\d{2})\s+at\s+(\d{1,2}:\d{2}\s+[AP]M)',
  );
  static final RegExp receiveMoneyRegex = RegExp(
    r'You\s+have\s+received\s+Ksh([0-9,.]+)\s+from\s+(.+?)\s+on\s+(\d{1,2}/\d{1,2}/\d{2})\s+at\s+(\d{1,2}:\d{2}\s+[AP]M)',
  );
  static final RegExp payBillRegex = RegExp(
    r'Ksh([0-9,.]+)\s+paid\s+to\s+(.+?)\.\s+on\s+(\d{1,2}/\d{1,2}/\d{2})',
  );

  static TransactionEntity? parseMessage(String body, String userId) {
    // Send Money / PayBill (Expense)
    var match =
        sendMoneyRegex.firstMatch(body) ?? payBillRegex.firstMatch(body);
    if (match != null) {
      ...
      return TransactionEntity(
        ...
        note: 'To: $recipient',
        date:
            DateTime.now(), // Approximate to today since full parsing requires complex date building
